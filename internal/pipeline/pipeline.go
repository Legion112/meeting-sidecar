...
type Deps struct {
	...
	HUD           ui.HUD
	CaptionSource ui.CaptionSource
	SampleRate    int
	SystemPrompt  string
	Logger        *slog.Logger
}
...
func (r *Runner) handleUtterance(ctx context.Context, pcm []int16) error {
	...
	if d.Logger.Enabled(ctx, slog.LevelDebug) {
		d.Logger.Debug("stt transcript", "source", d.CaptionSource, "text", text)
	}
	d.HUD.AppendCaption(d.CaptionSource, text)
	...
	if d.Logger.Enabled(ctx, slog.LevelDebug) {
		d.Logger.Debug("question gate", "text", text, "is_question", ok)
	}
	if !ok {
		d.HUD.SetStatus("listening")
		return nil
